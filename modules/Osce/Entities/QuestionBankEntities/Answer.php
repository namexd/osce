...
class Answer extends Model
{
    ...
    public function saveAnswer($data,$resultData)
    {
        ...
        try{
            ...
            //todo:gaodapeng 2016.07.06在将数据存入数据库之前判断成绩是否有效
            $flag = 0;
            if(!empty($examQueueInfo)){
                if($examQueueInfo->controlMark >0){
                    if($examQueueInfo->controlMark== 1){
                        $examQueueInfo->controlMark =2;
                    }
                    if($examQueueInfo->controlMark== 2){
                        $examQueueInfo->controlMark =3;
                    }
                    if($examQueueInfo->controlMark== 3){
                        $examQueueInfo->controlMark =4;
                    }
                    $flag = $examQueueInfo->controlMark;
                }
//                \Log::info($examQueueInfo,$resultData);
                //将向考试结果记录表增加一条数据
...
//                    if(!ExamResult::where('id','=',$examResultInfo['id'])->update($examResultData)){
                        foreach ($examResultData as $key =>$value){
                            $examResultInfo->$key = $value;
                            if(!$examResultInfo->save()){
        ...
        $DB->beginTransaction();
         try{
            ...
            if(!empty($quene)){
                ...
                $examResultModel->  NextExaminee($examId,$quene->exam_screening_id,$quene->room_id,$teacherId,$stationId);
