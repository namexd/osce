...
class SubjectItem extends CommonModel
{
    ...
    protected $guarded      = [];
    protected $hidden       = [];
    protected $fillable     = ['subject_id', 'content', 'sort', 'score', 'pid', 'level', 'created_user_id', 'answer','coefficient'];
    public    $search       = [];
        ...
        foreach($content as $prointIndex => $item)
        {
            if(empty($item['title'])){
                ...
                foreach($item as $contentIndex  =>  $content){
                    if($contentIndex=='title' || $contentIndex=='id'){
                        continue;
                    }

                    $contentData    =   [
                        'content'       =>  is_array($content)?$content[$contentIndex]:$content,        //判断是否为数组？根据序号取值:直接拿取
                        'score'         =>  array_key_exists($contentIndex,$itemScore)? $itemScore[$contentIndex]:0,
//                        'coefficient'   =>  floatval($itemScore['rate']),
                        'sort'          =>  $contentIndex,
                        'answer'        =>  array_key_exists($contentIndex,$itemAnswer)? $itemAnswer[$contentIndex]:''
                    ];
                    //判断是否为数组？保留原有ID:''
                    if(is_array($content) && array_key_exists('id', $content)){
