...
//            $user = $this->handleUser($userData);
    ...
    public function  AbnormalStudent($examId,$roomId , $screeningId){
        
        $DrawFinish = Student::leftjoin('exam_queue', function ($join) {
        ...
        })->leftjoin('exam_monitor', function ($join) {
            $join->on('exam_queue.exam_id', '=', 'exam_monitor.exam_id');
        })
            ->where('exam_queue.room_id', '=', $roomId)
            ->where('exam_queue.exam_id', '=', $examId)
            ->where('station_teacher.exam_id', $examId)
            ->where('exam_monitor.exam_id', $examId)
            ->where('exam_monitor.room_id', $roomId)
//            ->where('exam_queue.student_id', $student_id)
            ->where('exam_queue.exam_screening_id', $screeningId)
            ->whereIn('exam_queue.status',[1,2])
            ->whereIn('exam_queue.controlMark', [1,2,3])
            ->select([
    ...
    {
        //当前场次
        $exam_screening   =   Common::getExamScreening($exam->id);
        $exam_screening_id =  $exam_screening->id;
...
//                ->where('exam_queue.blocking', 1)
    ...
    {
        //当前场次
        $exam_screening_id = Common::getExamScreening($exam->id)->id;
...
//    }
    ...
    public function getStudentQueue($exam_id, $screen_id, $countStation)
    {
        ...
        $endStudentList = ExamQueue::where('exam_id', '=', $exam_id)
                            ...
                            ->whereIn('status', [0,1,2])->get();     //0:绑定腕表,1:抽签,2:正在考试,3:结束考试,4:缺考
        //判断是否有 已经绑定了腕表待考 或者 在考试中的学生
        if(count($endStudentList))
        {
            ...
            $builder = $this->leftjoin('exam_order', 'exam_order.student_id', '=', 'student.id')     //TODO wt 未绑定时队列表没数据
                            ->where('exam_order.exam_id', '=', $exam_id)
                            ->where('student.exam_id', '=', $exam_id)
                            ->where('exam_order.exam_screening_id', '=', $screen_id)
                            ->whereIn('exam_order.status', [0,2,4]);    //0:未绑定,1:绑定腕表,2:已解绑,3:缺考,4:跳过,5:替考
        ...
        }else
        {
            $builder = $this->leftjoin('exam_order', 'exam_order.student_id', '=', 'student.id')     //TODO wt 未绑定时队列表没数据
                            ...
                            ->where('student.exam_id', '=', $exam_id)
                            ->where('exam_order.exam_screening_id', '=', $screen_id)
                            ->whereIn('exam_order.status', [0,4]);      //0:未绑定,1:绑定腕表,2:已解绑,3:缺考,4:跳过,5:替考
        }

        $builder = $builder->select([
                                'student.name', 'student.idcard', 'student.code', 'exam_order.exam_screening_id',
                                'student.id',   'student.mobile', 'exam_order.status'
                            ])
                            ->orderBy('exam_order.begin_dt','asc')->paginate(50);
        return $builder;
    }
    ...
    {
        //todo:GaoDapeng 2016/06/16 科目成绩统计新增理论试卷详情查看
        ...
        return Student::leftJoin('exam_result', 'exam_result.student_id', '=', 'student.id')
            ...
            /*->where('station.paper_id', '=', $paperId)*/
            ...
            ->select(
                ...
                'exam_paper.length as pmins',
                'exam_screening.begin_dt as screen_begin',
                'exam_screening.end_dt as screen_end',
    ...
    static public function getSubjectData($examId, $subjectId)
    {
        return Student::leftJoin('exam_result', 'exam_result.student_id', '=', 'student.id')
            ->leftJoin('exam_screening', 'exam_screening.id', '=', 'exam_result.exam_screening_id')
            ->leftJoin('exam', 'exam.id', '=', 'exam_screening.exam_id')
...
//                'exam_result.time as exam_result_time',
    ...
    static public function getStudentScoreList($screening_ids, $message)
    {
        ...
        $builder = $builder->select(
            ...
            'student.code as student_code',
            'exam.name as exam_name',
            DB::raw('sum(exam_result.score) as score_total'),
            DB::raw('count(*) as station_total')
        )
            ->where('exam.status', '<>', 0)
    ...
    public static function getStudentScoreRank($exam_id, $total_score)
    {
        $builder = ExamResult::leftJoin('exam_screening', 'exam_screening.id', '=', 'exam_result.exam_screening_id')
            ->where('exam_screening.exam_id', '=', $exam_id)
            ->select(DB::raw(implode(',',
                [
                    'count(*) as rank',
                ])))
            ->having(DB::raw("sum(exam_result.score) >= $total_score"))
            ->groupBy('exam_result.student_id')->first();
        return $builder;
    }
     ...
     * @time 2016-04-21
     */
    public function sendSms($notice,$to,$url,$type ='msg')
    {
...
//            fclose($f);
    ...
    public function sendEmail($notice,$to,$url){
        try {
            ...
            $content=   [];
            $content[]  =   '亲爱的'.$notice->student->name.'同学:';
            $content[]  =   '你有一场考试为'.$notice->exam->name. ' ' .'考试开始时间为'.$notice->student->student_begin_dt;
            $content[]  =   '请你准时参加'.$url;
...
//                    $sender->send($value->student->mobile,$str);
                    ...
                    if($sendType['student']['mail'] == 1){
                        $type = 'mail';
                        $this->sendSms($value, $studentOrder->student->userInfo->email, $url,$type);
                    }
                    
                    if($sendType['student']['mail'] == 1){
                        $this->sendEmail($value, $studentOrder->student->userInfo->email, $url);
                    }
