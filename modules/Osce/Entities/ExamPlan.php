...
//                        dump($batch);
            $timeData[strtotime($record->begin_dt)][$record->room_id]['name'] = $name;
            $timeData[strtotime($record->begin_dt)][$record->room_id]['start'] = strtotime($record->begin_dt);
    ...
    //获取当前考生
    public function showSpeechNow($exam_id,$num){
        if(!isset($num)){
            $exam_id=1;
        }
