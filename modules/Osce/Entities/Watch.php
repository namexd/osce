...
//            }
    ...
    public function getWatchAboutData($status,$type,$nfc_code,$examId){
        ...
        $builder = $builder->where('watch.status','=',$status)->where('exam_screening_student.is_end','=',0)->whereNotin('exam_screening_student.status',[1,2,3,4,5,6])->where('watch_log.action','=','绑定')->where('exam_queue.exam_id','=',$examId)->leftjoin('watch_log',function($watchLog){
