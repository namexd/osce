...
//            ])->get();
    ...
    static public function examineeByRoomId($room_id, $examId, $stations, $exam_screening_id)
    {
        try {
            ...
            //没有正在考试的
            if ($queueing->count() == count($stations)) {
...
//                    ->get();
    ...
    {
        \Log::info('获取考场下一组所需参数',[$room_id, $examId, $station, $exam_screening_id]);
        try {
...
//                ->first();
            ...
            \Log::info('下一组排出的学生',[$room_id,$studentIds]);
            $nextExamQueue =  ExamQueue::leftJoin('student', 'student.id', '=', 'exam_queue.student_id')
                ->where('exam_queue.room_id', $room_id)
                ->where('exam_queue.status', '<', 3)
                ->where('exam_queue.exam_id', $examId)
                ->where('exam_queue.exam_screening_id', $exam_screening_id)
                ->where('exam_queue.blocking', 1)
                ->whereNOtIn('exam_queue.student_id',$studentIds)
                ->whereNOtIn('student.id',$studentIds)
//                ->skip(count($station))
                ->take(count($station))
                ->orderBy('exam_queue.next_num', 'asc')
                ->orderBy('exam_queue.begin_dt', 'asc')
//                ->orderBy('exam_queue.id', 'asc')
                ...
                ->groupBy('student.id')
                ->get();
            \Log::info('缓存时每一个考场的下一组',[$nextExamQueue]);
...
//        dd($nowQueue);
    ...
    public function AlterTimeStatus($studentId, $stationId, $nowTime, $teacherId, $examscreeningId)
    {
        ...
        try {
            ...
            //查询学生是否已开始考试
            //dd($studentId);
            \Log::alert('开始考试数据',[$studentId,$stationId,$exam->id,$examscreeningId]);
...
//            CommonController::storeAnchor($stationId, $studentId, $exam->id, $teacherId, [$nowTime]);
     ...
     * @return
     * @throws \Exception
     * @author Jiangzhiheng  zhoufuxiang<zhoufuxiang@163.com>
     */
    static public function findQueueIdByStudentId($studentId, $stationId)
    {
        try {
            ...
            $examScreen_id = $examScreening->id;

            //通过学生id找到对应的examScreeningStudent实例
            $ScreenStudent = ExamScreeningStudent::where('student_id', '=', $studentId)->where('exam_screening_id', '=', $examScreen_id)->first();

            \Log::alert('结束考试场次信息',['screening_id' => $examScreen_id, 'exam_screening_student_id' => $ScreenStudent->id]);
            if (is_null($ScreenStudent)) {
                throw new \Exception('没找到对应的学生编号', 2100);
            }

            //获取考生正在进行考试的队列信息
            $queue = ExamQueue::where('student_id', $studentId)
                    ->where('exam_screening_id', $examScreen_id)
                    ->where('station_id', $stationId)
                    ->first();
            if (is_null($queue)) {
            ...
            \Log::alert('结束考试改变学生的队列',['queue_id' => $queue->id, 'student_id' => $queue->student_id, 'queue_status' => $queue->status]);
            return $queue;
...
//                        ->paginate($pageSize);
    ...
    {
        $builder = $this->leftjoin('exam', 'exam.id', '=', 'exam_queue.exam_id')
                        ->leftjoin('station', 'station.id', '=', 'exam_queue.station_id')
                        ->where('exam_queue.exam_id', '=', $examId)
                        ->where('exam_queue.student_id', '=', $studentId)
                        ->where('station.type', '=', 3)
                        ->select('exam.id', 'exam.name', 'exam_queue.station_id', 'exam_queue.status', 'exam_queue.room_id', 'station.paper_id')
                        ->orderBy('exam_queue.begin_dt', 'asc')->get();
        return $builder;
    }
...
//            return \Response::json(array('code' => 2));     //没有对应的开考场次 —— 考试场次没有(1、0)
    ...
    public function getCurrentGroupStudent($user_id, $exam_id)
    {
        try{
            ...
            //3、获取当前老师、在当前场次下支持的考站，考站所在的考场，
            $ExamDraft = new ExamDraft();
            list($stationId, $roomStations) = $ExamDraft->getStationAndRoom($user_id, $exam->id, $screening_id);
            //判断获取到的考站、考场是否为空
            if ($roomStations->isEmpty()) {
                throw new \Exception('未能查到该老师对应的考场！', -733);
            }
            $room_id = $roomStations->first()->room_id;              //获得考场的id
            //4、获得当前考场考站的实例列表
            $stationIds = $roomStations->pluck('station_id')->toArray();
            $stations = Station::whereIn('id', $stationIds)->get();

            //5、根据考试模式 获取当前考生队列
            switch ($exam->sequence_mode) {
                case 1:
                    $examQueue = ExamQueue::examineeByRoomId($room_id, $exam->id, $stations, $screening_id);
                    break;
                case 2:
                    $examQueue = ExamQueue::examineeByStationId($stationId, $exam->id, $screening_id);
                    break;
                default:
    ...
    public function getNextGroupStudent($user_id, $exam_id)
    {
        try{
            ...
            switch ($exam->sequence_mode) {
                case 1:
                    $examQueue = ExamQueue::nextExamineeByRoomId($room_id, $exam->id, $stations, $screening_id);
                    break;
                case 2:
                    $examQueue = ExamQueue::nextExamineeByStationId($stationId, $exam->id, $screening_id);
                    break;
                default:
                    throw new \Exception('考试模式不存在！', -703);
            }
    ...
    {
        try{
            \Log::info($room_id.'考场当前组更新缓存获取到的考试id',[$exam_id]);
            //获取正在考试中的考试
            $exam = Common::examCheck($exam_id);
            //重新获取当前场次
            $examScreening = Common::getExamScreening($exam_id);
            $ExamDraft  = new ExamDraft();
            $stations   = $ExamDraft->getExamMsg($exam_id, $room_id, $examScreening->id);   //对应考试中room_id下所有考站
            //根据考试模式获取对应的当前组队列
            switch ($exam->sequence_mode)
            {
                case 1:
                    $examQueue = ExamQueue::examineeByRoomId($room_id, $exam->id, $stations, $examScreening->id);
                    break;
                case 2:
                    $examQueue = $this->getAllQueueByStation($stations, $exam->id, $examScreening->id);
                    break;
                default:
                    throw new \Exception('考试模式不存在！', -703);
            }
    ...
    public function getAllQueueByStation($stations, $exam_id, $screening_id, $param = 'current')
    {
        ...
        if(!$stations->isEmpty()) {
            foreach ($stations as $value) {
                if($param == 'current')     //当前组
                {
                    $singleQueue = ExamQueue::examineeByStationId($value->station_id, $exam_id, $screening_id);
                }else{
                    $singleQueue = ExamQueue::nextExamineeByStationId($value->station_id, $exam_id, $screening_id);
                }
                $examQueue->push($singleQueue);
            }
        }
        return $examQueue;
    }
    ...
    {
        try{
            \Log::info($room_id.'考场下一组更新缓存获取到的考试id',[$exam_id]);
            //获取正在考试中的考试
            $exam = Common::examCheck($exam_id);
            //重新获取当前场次
            $examScreening = Common::getExamScreening($exam_id);
            ...
            {
                case 1:
                    $examQueue = ExamQueue::nextExamineeByRoomId($room_id, $exam->id, $stations, $examScreening->id);
                    break;
                case 2:
                    $examQueue = $this->getAllQueueByStation($stations, $exam->id, $examScreening->id, 'next');
                    break;
                default:
                    throw new \Exception('考试模式不存在！', -703);
            }
