...
class Exam extends CommonModel
{
    ...
    protected $hidden       = [];
    protected $fillable     = [
        'code', 'name', 'pid', 'begin_dt', 'end_dt', 'status', 'total', 'create_user_id',
        'description',
        'sequence_cate', 'sequence_mode', 'rules', 'address', 'teacher_arrange',
        'arrangement', 'same_time', 'stage', 'real_push', 'archived'
    ];
     ...
     * @throws \Exception
     */
    public function showExamList($formData = '', $id = '', $sign = false)
    {
        try {
            //不寻找已经被软删除的数据
            ...
            if($id){
                $builder = $builder->where('id', '<>', intval($id));
            }
    ...
    public function stopExamScreen($examId, $examScreenId) {
        ...
        try {
            ...
            if ($examScreen && $examScreen->status != 2) {
                $examScreen->status = 2;
                $examScreen->save();
                $this->openNextScreen($examScreenId);
                $connection->commit();
            }
        }
        catch (\Exception $e) {
    ...
    public function openNextScreen($examScreenId) {
        $examScreen = ExamScreening::find($examScreenId);
        $examId = $examScreen->exam_id;
        // 查看是否有下一场考次
...
//            dd($examData);
    ...
    public function editExam($exam_id, array $examData, array $examScreeningData, $gradation, $gradationMode, ExamArrangeRepository $examArrangeRepository)
    {
        ...
        try {
            //判断父考试
            if($examData['pid'] != 0 && $examData['pid'] == $exam_id){
                throw new \Exception('父考试不能选自己');
            }
            //更新考试信息
...
//                    dd($gradationData);
    ...
    {
        //todo:GaoDapeng 2016/06/16 将理论考试试卷加入科目统计
        //todo:GaoDapeng 2016/06/20 统计需剔除弃考的学生，判定是否子考试，仅显示最近一场的成绩
        //todo:GaoDapeng 2016/06/22 使用examScreeningId代替examId来获取所有考试包含子考试信息
        
        /*$builder    = ExamResult::leftJoin('exam_score', 'exam_score.exam_result_id', '=', 'exam_result.id')
                    ->leftJoin('subject', 'subject.id', '=', 'exam_score.subject_id');*/

        $builder = ExamResult::leftJoin('subject', 'subject.id', '=', 'exam_result.subject_id')
                ->leftJoin('station', 'station.id', '=', 'exam_result.station_id')
                ->leftjoin('exam_paper','exam_paper.id','=','station.paper_id');
        
        if ($screening_ids != ''){
                if(is_array($screening_ids)){
                    $builder = $builder->whereIn('exam_result.exam_screening_id', $screening_ids);
                }else{
                    $builder = $builder->where('exam_result.exam_screening_id', $screening_ids);
                }
        }

        if ($subjectId != ""){
            //通过$sign判定$itemId是否为理论考试试卷id
            if('paper' == $sign){
                $builder = $builder->where('exam_paper.id', '=', $subjectId);
            }else{
        ...
        $builder = $builder->select(
                ...
                ->where('exam_result.flag','=',0)//此条件判定成绩是否统计有效
    ...
    static public function doingExam($examId = null)
    {
        try {
            ...
            if (is_null($examId))
            {
                ...
                \Log::alert('根据考试ID获取考试',['exam_id'=>$examId, 'exam'=>$exam]);
                return $exam;
            }
