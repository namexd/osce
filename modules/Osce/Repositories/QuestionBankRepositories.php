    ...
    public function GetExamInfo($userId){
        try{
            ...
            $builder = $Exam->leftJoin('station_teacher', function($join){
                ...
                ->leftJoin('exam_gradation', 'exam_gradation.exam_id', '=', 'exam.id')
...
//                ->where('station_teacher.exam_screening_id',$exam_screen_id)   //TODO: Zhoufuxiang,不能用场次ID做条件查询
    ...
    public function getTeacherStation($user_id, $exam_id, $examScreening)
    {
        try{
            //根据监考老师的id和考试id，获取对应的考站id
            $result = StationTeacher::leftJoin('exam', 'exam.id', '=', 'station_teacher.exam_id')
                                    ->leftJoin('exam_gradation', 'exam_gradation.exam_id', 'exam.id')
            ...
            $station_id = $result->first();
            //添加日志
            \Log::debug('根据监考老师id获取相关信息', ['user_id'=>$user_id, 'screening_id'=>$examScreening->id, 'station_id'=>$station_id]);

            //获取对应考站的详细信息
            $station = Station::where('id', '=', $station_id->station_id)->first();
            if(is_null($station)){
                throw new \Exception('没有相应的考站详细信息',-1067);
            }
            //判断是否为理论考站
            if($station->type != 3) {
            ...
            //返回对应考站ID
            return  [$result->pluck('station_id')->toArray(), $station_id->station_id];
