...
class ExamController extends CommonController
{
    ...
    public function postAddExam(Request $request, Exam $model, ExamArrangeRepository $examArrangeRepository)
    {
        ...
        try{
            ...
            $examData = [
                ...
                'pid'            => intval($request->get('pid'))? :0,   //父考试ID
    ...
    public function getEditExam(Request $request)
    {
        ...
        try {
            ...
            //查询父考试
            if($examData->pid !=0){
                $parentExam = Exam::where('id', '=', $examData->pid)->first();
                $examData->pidExam = $parentExam->name;
            }
...
//            }
            ...
            $result = $addAllExamineeRepository->importStudent($exam, $student);
    ...
    {
        $formData['exam_name'] = $request->get('q');
        $exam_id   = $request->get('id');
        $sign   = $request->get('sign');
        $exam   = new Exam();
        $pagination = $exam->showExamList($formData, $exam_id, $sign);

        $data   =   $pagination->toArray();
        if($sign){
            return response()->json(
                $this->success_rows(1,'获取成功','','','', $data)
            );
        }else{
            return response()->json(
...
//        $student['exam']=Exam::where('exam_id',$student['exam_id'])->select()->first(); //查询准考证号
    ...
    public function getSpeechNow(Request $request){
        ...
        //dd($screen);
        if($screen){
            $minscreenid = ExamScreening::where('gradation_order',$screen->gradation_order)->where('exam_id',$exam_id)->min('id');
            $plan = ExamPlan::where(['exam_id'=>$exam_id,'exam_screening_id'=>$minscreenid])->orderBy('begin_dt','desc')->select('room_id')->first();
            $station = StationTeacher::where(['exam_id'=>$exam_id,'exam_screening_id'=>$minscreenid])->get();
            //dd($station);
