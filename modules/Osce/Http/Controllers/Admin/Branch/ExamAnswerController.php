...
class ExamAnswerController extends CommonController
{
     ...
     * @access public
     * @param
     * @author yangshaolin <yangshaolin@163.com>
    ...
    public function  getStudentAnswer(Request $request)
    {
        ...
        $id = $request->get('student_id');//学生id
        $flag = $request->get('flag');//学生id
        ...
        //todo:GaoDapeng 2016/06/21 将station_id加入判断条件 修正了试卷总分的显示错误
        $station_id = Station::leftjoin('exam_paper','exam_paper.id','=','station.paper_id')
            ->where('exam_paper.name','=',$examItems['exam_name'])
            ->select('station.id')->pluck('id');
        $examItems['stuScore'] =ExamResult::where('student_id',$studentMsg->id)
