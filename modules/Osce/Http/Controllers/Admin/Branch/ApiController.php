...
//-_-------------------------------------------
     ...
     *
     * @version 1.0
     * @author wangjiang <wangjiang@163.com>
    ...
    public function LoginAuthWait(QuestionBankRepositories $questionBankRepositories, ExamMidwayRepository $examMidway, DrawlotsRepository $drawlots)
    {
        try {
            ...
            try{
                ...
                $stationIds = array_column($stationIds->toArray(),'station_id');
                //拿到场次下房间里该老师支持的考站
    ...
    public function LoginAuthWait2(QuestionBankRepositories $questionBankRepositories, ExamMidwayRepository $examMidway, DrawlotsRepository $drawlots)
    {
        try {
            ...
            //1、获取本次考试的id
            $examInfo = Exam::doingExam();
            $exam_id  = $examInfo->id;      //获取考试ID

            //2、获取当前正在考试的场次
            $examScreening = Common::getExamScreening($exam_id);
            $screening_id  = $examScreening->id;        //获取场次ID

            //根据监考老师的id，获取对应的考站id
            list($station_ids, $stationId) = $questionBankRepositories->getTeacherStation($user->id, $exam_id, $examScreening);
            ...
            try{
                //获取考场
                $exam_room  = ExamDraft::getExamRoom($exam_id, $screening_id, $station_ids);
                //获取考场下所有的考站
                $stationIds = $drawlots->getStationNum($exam_id, $exam_room->room_id, $screening_id);
                $stationIds = array_column($stationIds->toArray(),'station_id');
                //拿到场次下房间里该老师支持的考站（取交集）
                $station_id = array_intersect($station_ids, $stationIds);

                //改变老师支持该考站的准备状态(状态改为1)
                $ExamStationStatus = new ExamStationStatus();
                $ExamStationStatus ->getStationStatus($exam_id, $station_id, $screening_id);

                //检查是否要把状态变更为2
                if ($examMidway->isChangeToTwo($exam_id, $stationIds)) {
                    //就把所有考站改为2
                    $ExamStationStatus->getStationStatus($exam_id, $stationIds, $screening_id, $type =2);
                }
            }catch (\Exception $exe){
                \Log::debug('理论考试老师登陆改变准备状态出错',[$data,]);
            }

            return view('osce::admin.theoryCheck.theory_check_volidate', ['data' => $data]);
...
//                    }
    ...
    public function getStudentExamIndex2(ExamMidwayRepository $examMidway, DrawlotsRepository $drawlots)
    {
        $user = Auth::user();
        //查找当前正在进行的考试--之后会改
        $exam = Exam::doingExam();
        $exam_id = $exam->id;       //获取考试ID
        $Student = new Student();
        //获取用户的信息及已报的考试
        $studentInfo = $Student->getStudentExamInfo($user->id, $exam_id);
        if(is_null($studentInfo)){
            throw new \Exception('没有找到该考生的考试信息', -203);
        }
        //在队列表中查找与考试相关的数据
        $ExamQueue = new ExamQueue();
        $examQueueInfo = $ExamQueue->getExamingData($exam_id, $studentInfo->id);

        //整理考试数据
        $examData = [];
        $StationTeacher = new StationTeacher();

        foreach($examQueueInfo as $key => $item)
        {
            $stationTeacher = $StationTeacher->where('station_id', '=', $item->station_id)->where('exam_id', '=', $exam_id)->first();

            $examData[$key]['station_id']   = $item->station_id;
            $examData[$key]['teacher_id']   = $stationTeacher->user_id;
            $examData[$key]['student_id']   = $studentInfo->id;
            $examData[$key]['paper_id']     = $item->paper_id;
            $examData[$key]['exam_id']      = $exam_id;
            $examData[$key]['exam_name']    = $exam->name;
            $examData[$key]['status']       = $item->status;
        }

        //获取考生当前是在哪个考站考试
        $queue = $examMidway->getQueueByStudent($studentInfo->id, $exam_id);
        //检查此考站是否是理论考站
        if (Station::find($queue->station_id)->type != 3) {
            throw new \Exception('学生当前不应该考这个考站', -111);
        }
        //修改exam_station_status表状态
        $examMidway->beginTheoryStatus($exam_id, [$queue->station_id], 1);
        /*
         * 判断是否要把1改成2
         */
        $screen   = $drawlots->getScreening($exam_id);
        $stations = $drawlots->getStationNum($exam_id, $queue->room_id, $screen->id);
        //判断当前已经有多少个考站已经是1了
        $bool     = $examMidway->isChangeToTwo($exam_id, $stations->pluck('station_id')->toArray());
        if ($bool) {
            $examMidway->beginTheoryStatus($exam_id, $stations->pluck('station_id')->toArray(), 2);
        }
    ...
    public function getExamPaperStatus(Request $request,DrawlotsRepository $draw)
    {
        ...
        try {
            ...
            //检查当前组异常考生
            if(!$unExamQueues->isEmpty()) {
                $ExamQueue = $unExamQueues->first();
                $request['room_id'] = $ExamQueue->room_id;
                //检查异常考生抽签
                $abnormal =1;
                //拿到异常考生
                ...
                if (!empty($examQueue) || count($examQueue) > 0) {
                    ...
                    if($abnormal == 1){
                        //把异常考生抽签
                        $draw->setParams($request->all());
                        $draw->UnusualStudentDraw();
                    }
                }
            }
    ...
    public function getReadyExam (Request $request, WatchReminderRepositories $watchReminder, DrawlotsRepository $draw)
    {
        ...
        try{
            if(!empty($examQueue) || count($examQueue)>0){
                ...
                //2、查询当前老师对应考站准备完成信息，判断是否准备完成
                $ExamStationStatus = new ExamStationStatus();
                $ExamStationStatus->getStationReadyStatus($examId, $examScreeningId, $stationId);

                if($abnormal ==2){
                    // 3.调用检查腕表信息
                    $watchLogModel = new WatchLog();
                    $watchLogModel->getRoomStudentWatchs($examId, $examScreeningId, $roomId);
                }else{
                    //4,把异常考生抽签
                    $draw->UnusualStudentDraw();
                }
                ...
                //5、查询exam_station_status表（考试-场次-考站状态表）中该考试该考场下status是否全为1，如果是，修改其状态值为2
                 $ExamStationStatus->modifyExamStationStatus($examId, $roomId, $examScreeningId, $draw);
                $request['station_id']  = $stationId;
                $request['teacher_id']  = $teacherId;
                $request['exam_id']     = $examId;
                $request['id']          = $teacherId;
                Common::pushStudent($examId,$teacherId);
                $inv=new InvigilatePadController();
                $inv->getAuthentication_arr($request);//当前考生推送(如果有)
            ...
            return response()->json(
                $this->success_data([], $ex->getCode(), $ex->getMessage())
            );
    ...
    /**
     * 监考老师pad端点击准备完成，并给腕表推送消息
     * @param Request $request
     * @param WatchReminderRepositories $watchReminder
     * @param DrawlotsRepository $draw
     * @return \Illuminate\Http\JsonResponse
     */
    public function getReadyExam2(Request $request, DrawlotsRepository $draw, WatchReminderRepositories $watchReminder)
    {
        try{
            \Log::alert('老师准备传入所有的参数', $request->all());
            $this->validate($request, [
            ...
            //3、查询当前老师对应考站准备完成信息，判断是否准备完成
            $ExamStationStatus = new ExamStationStatus();
            $examStationStatus = $ExamStationStatus->getStationReadyStatus($examId, $examScreeningId, $stationId);
            ...
            {
                case 1:
                    $watchCodes = $WatchLog->getRoomStudentWatchs($examId, $examScreeningId, $roomId);
                    break;
                case 2:
                    $watchCodes = $WatchLog->getStationStudentWatch($examId, $examScreeningId, $stationId);
                    break;
                default:
                    throw new \Exception('没有对应的考试模式', -5);
            }

            //5、查询exam_station_status表（考试-场次-考站状态表）中该考试该考场下status是否全为1，如果是，修改其状态值为2
            $result = $ExamStationStatus->modifyExamStationStatus($examId, $roomId, $examScreeningId, $draw);
