        ...
        switch ($status){
            ...
            case 2://替考
                ...
                foreach($list as $key=>$v) { //替考学生
                    //查询标记替考的考站
                    $replaceList=ExamMonitor::where('student_id',$v['student_id'])->where('exam_id',$exam_id)->where('exam_screening_student',$ExamScreening)->where('type',1)->get()->toArray();//上报停考信息
                    //print_r($replaceList);
