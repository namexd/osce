...
class IndexController extends CommonController
{
    ...
    public function getWatchStatus2(Request $request)
    {
        ...
        if (is_null($watch)) {
            return \Response::json(array('code' => 3, 'message' => '没有对应腕表信息'));     //数据库无腕表
        }
        $id       = $watch->id;           //获取腕表ID
    ...
    {
        \Log::info('绑定腕表（原始接口）请求的参数',$request->all());
        $this->validate($request, [
...
//            }
    ...
    {
        \Log::info('绑定腕表请求的参数',$request->all());
        $this->validate($request, [
            'code'      => 'required',  //腕表code
            'id_card'   => 'required',  //身份证号
            'exam_id'   => 'required'   //考试id
        ...
        try{
            ...
            //3、学生处理，（验证学生是否参加了当前考试）
            //3.1查询学生是否参加当前考试
            $studentExam= Student::where('idcard', '=', $id_card)->where('exam_id', '=', $exam_id)->first();
            $this->judgeNullValue($studentExam, 3, '没有参加当前考试');   //没有参加当前考试
           ...
           $MonitorStudent =  $ExamMonitorModel ->ExamMonitor($exam_id,$student_id,$examScreeningId);
            ...
            //3.2查询该学生是否在 当前考试的排考计划中
            $planId = ExamPlan::where('student_id', '=', $student_id)->where('exam_screening_id', '=', $examScreeningId)
                              ->where('exam_id','=',$exam_id)->select('id')->first();
            $this->judgeNullValue($planId, 4, '未安排当前考试');         //未安排当前考试

            //3.3获取考试队列中(exam_order)的考生列表
            $students   = $this->getStudentList($request);
            $idcards    = [];
            $students   = json_decode($students->content());
            //判断当前考生，是否在队列中
            ...
            if(!in_array($id_card, $idcards)){
                throw new \Exception('未在考试队列 请等待', 5);   //未在考试队列 请等待
            }

            //4、腕表处理
            //4.1检查腕表是否存在
            $watch  = Watch::where('code', '=', $code)->first();
            $this->judgeNullValue($watch, 111, ' 当前腕表不存在');       //当前腕表不存在
            //获取腕表id
            $watchId = intval($watch->id);

            //4.2判断腕表是否已绑定并且没有解绑 **/
            //取腕表最后一次使用记录
            $watchLog = WatchLog::where('watch_id', '=', $watchId)->orderBy('created_at', 'desc')->first();
            if(!is_null($watchLog)){
                if($watchLog->action == '绑定'){
                    throw new \Exception('当前腕表已绑定身份证', 11); //判断当前腕表已绑定身份证
                }
            }
            //4.3修改腕表状态
            $watch->status = 1;
            if($watch->save() === false){
                throw new \Exception('腕表绑定失败', 0);    //腕表状态修改失败
            }
            ...
            //6、签到（根据场次签到）
            $examScreeningStudent = ExamScreeningStudent::where('student_id', '=', $student_id)
                                  ->where('exam_screening_id', '=', $examScreeningId)->first();
            //6.1首次签到，创建场次学生数据
            if (is_null($examScreeningStudent)) {
                $screeningStudentData = [
                    'watch_id'          => $watchId,
             ...
             //6.2再次签到，更新场次学生数据（重新绑腕表）
            } else
            {
                $examScreeningStudent -> is_end     = 0;
                $examScreeningStudent -> watch_id   = $watchId;
                $examScreeningStudent -> signin_dt  = $updated_at;
                if (!$examScreeningStudent->save()) {
                    throw new \Exception('再次签到失败！');
                }
            }

            //7、更改考生为绑定腕表状态（0:未绑定,1:绑定腕表,2:已解绑,3:缺考,4:跳过,5:替考）
            ExamOrder::where('exam_id', $exam_id)->where('student_id', $student_id)
                     ->where('exam_screening_id', '=', $examScreeningId)->update(['status' => 1]);
            //8、考试开考（0:未开考,1:正在考试,2:考试结束）
            Exam::where('id', '=', $exam_id)->update(['status' => 1]);  //更改考试状态（把考试状态改为正在考试）

            //9、更新所有当前组、下一组缓存(1、根据老师缓存当前组、下一组，2、根据考场缓存当前组、下一组）
            Common::updateAllCache($exam_id, $examScreeningId);

            //10、腕表推送 TODO: 绑定腕表调腕表接口
//            $examQueue = new ExamQueue();
//            $WatchReminderRepositories = new WatchReminderRepositories();
//            try{
//                $data =  $examQueue-> getStudentWatchMovement($exam_id,$student_id,$examScreening);
//
//                foreach ($data as $value){
//                    \Log::info('绑定腕表学生id',[$value]) ;
//                    $WatchReminderRepositories->getWatchPublish($exam_id,$value);
//                }
//            }catch (\Exception $exe){
//                \Log::debug('绑定腕表调腕表接口出错',[$student_id, $exe]);
//            }
...
//                    }
     ...
     *
     * @version 1.0
     * @author limingyao <limingyao@163.com>
     * @date 2016-01-12
     * @copyright 2013-2015 MIS 163.com Inc. All Rights Reserved
     */
     ...
     *
     * @version 1.0
     * @author zhouchong <zhouchong@163.com> zhoufuxiang
     * @date ${DATE} ${TIME}
     * @copyright 2013-2015 MIS 163.com Inc. All Rights Reserved
     */
...
//        }
        ...
        try {
            ...
            $countStation = $stations->pluck('station_id')->toArray();
            if(!(count($countStation) > 0)){
...
//            $data = $list->toArray();
    ...
    public function getSkipLast(Request $request){
        ...
        //当前场次
        $screen_id = Common::getExamScreening($exam_id);
...
//        $examScreening  -> getExamCheck();
    ...
    public function changeSkip($studentId,$exam_id,$screen_id)
    {
        ...
        //查看这个学生在该场次是否弃考
        $ExamMonitorModel =  new ExamMonitor();
        $MonitorStudent =  $ExamMonitorModel ->ExamMonitor($exam_id,$studentId,$screen_id);
        if(!$MonitorStudent->isEmpty()){
//            throw  new \Exception('该学生已弃考',-100);
