...
@section('content')
    <input type="hidden" id="parameter" value="{'pagename':'examinee_manage_add','preUrl':'{{route('osce.admin.exam.getExamineeManage')}}?id={{$id}}','code':'{{route("osce.admin.exam.postExamSequenceUnique")}}','idcard':'{{route("osce.admin.exam.postExamSequenceUnique")}}','exam_sequence':'{{route("osce.admin.exam.postExamSequenceUnique")}}','mobile':'{{route("osce.admin.exam.postExamSequenceUnique")}}','header':'{{ url('commom/upload-image') }}'}"/>
