                        ...
                        @if(!empty(@$data['list']))
                            @foreach(@$data['list'] as $k=>$v)
                                <tr>
                                    ...
                                    <td>{{@$v['time']}}</td>
                                    <td>{{@$v['timeAvg']}}</td>
                                    <td>{{@$v['scoreAvg']? :'0.00'}}分</td>
                                    <td>{{@$v['mins']}}</td>
                                    <td>{{@$v['score']? :'0.00'}}分</td>
                                    <td>
                                        @if($v['type']==3)
                                            <a href="/osce/admin/examanswer/student-answer?student_id={{$v['student_id']}}">
                                                <span class="read  state1 detail"><i class="fa fa-search fa-2x"></i></span>
                                            </a>
                                        @else
                                        <a href="/osce/admin/exam/exam-result-detail?exam_result_id={{$v['result_id']}}">
                                            <span class="read  state1 detail"><i class="fa fa-search fa-2x"></i></span>
                                        </a>
                                        @endif
                                    </td>
