...
#define SYSTRANS_E_MUXER        0x80000010  // 底层mp4打包库打包错误
...
{
	unsigned char*  pSrcInfo;       //海康设备出的媒体信息头（源码流信息）
	unsigned int    dwSrcInfoLen;   //当前固定为40
	SYSTEM_TYPE     enTgtType;      //目标封装
	unsigned int    dwTgtPackSize;  //设置为0时，使用库内默认值。如果目标为RTP，PS/TS等封装格式时，设定每个包大小的上限。如果目标为AVI，设定最大帧长
} SYS_TRANS_PARA;
...
typedef struct OUTPUTDATA_INFO
{
	...
	unsigned int    dwFlag;             //输出数据是否mp4索引0:否；1：是/// mp4索引前置新增变量
} OUTPUTDATA_INFO;
...
extern "C" {
	...
	*        dwUser             - 用户数据
	* 返回值：状态码
	* 说明：  输出数据的详细信息,支持目标封装为PS/TS/RTP/MP4/FLV/ES
	************************************************************************/
