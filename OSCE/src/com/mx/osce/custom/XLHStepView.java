...
public class XLHStepView extends View {
	...
	public XLHStepView(Context context, AttributeSet attrs) {
		super(context, attrs);
		TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.xlhStep);
		mBarColor = typedArray.getColor(R.styleable.xlhStep_barColor, Color.BLUE);
		mStepSelectedColor = typedArray.getColor(R.styleable.xlhStep_stepSelectedColor, Color.RED);
		mStepNormalColor = typedArray.getColor(R.styleable.xlhStep_stepNormalColor, Color.BLUE);
		mProgressColor = typedArray.getColor(R.styleable.xlhStep_progressColor, Color.RED);
	...
	private void drawSteps(Canvas canvas) {
		...
		float barWidth = getMeasuredWidth() - mStepRadius * 2 - getPaddingLeft() - getPaddingRight();// 背景条的总宽度
		for (int i = 0; i < mStepCount; i++) {
			int pivotX = (int) (mStepRadius + getPaddingLeft() + i * barWidth / (mStepCount - 1));// 当前节点圆的中心点X坐标
			// 下面开始画圆
			...
			RectF oval = new RectF(pivotX - mStepRadius, getPaddingTop(), pivotX + mStepRadius,
					getPaddingTop() + mStepRadius * 2);
	...
	private void drawBarBg(Canvas canvas) {
		...
		RectF r = new RectF(getPaddingLeft() + mStepRadius, getPaddingTop() + mStepRadius - mBarHeight / 2,
				getMeasuredWidth() - getPaddingRight() - mStepRadius, getPaddingTop() + mStepRadius + mBarHeight / 2);
	...
	private void drawProgress(Canvas canvas) {
		...
		RectF r = new RectF(getPaddingLeft() + mStepRadius, getPaddingTop() + mStepRadius - mBarHeight / 2,
				getPaddingLeft() + mStepRadius + barWidth * (mProgress),
				getPaddingTop() + mStepRadius + mBarHeight / 2);
		canvas.drawRect(r, paint);
		canvas.restore();
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (heightMode == MeasureSpec.AT_MOST) {
			heightSize = (int) mStepRadius * 2 + getPaddingBottom() + getPaddingTop();
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (mCanDrag) {// 支持拖动
			...
			if (x < getPaddingLeft() + mStepRadius || x > getMeasuredWidth() - getPaddingRight() - mStepRadius) {
				return true;
			}
			...
			case MotionEvent.ACTION_UP:
				...
				for (int i = 0; i < mStepCount; i++) {
					if (minDiatance >= (Math.abs(mProgress - i / (float) (mStepCount - 1)))) {
						minDiatance = Math.abs(mProgress - i / (float) (mStepCount - 1));
						temp = i;
					}
				}
	...
	public void setStepChangedListener(OnStepChangedListener mStepChangedListener) {
