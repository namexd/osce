package com.mx.bluetooth.audio;

import java.io.File;

import com.mx.bluetooth.R;
import com.mx.bluetooth.audio.AudioManage.AudioStateListenter;
...
public class AudioRecorderButton extends Button implements AudioStateListenter {
	...
	public AudioRecorderButton(Context context, AttributeSet attrs) {
		...
		String dir = Environment.getExternalStorageDirectory() + File.separator + context.getPackageName()
				+ File.separator + "AudioRecord/"; // 此处需要判断是否有存储卡(外存)
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			/*
			 * else if (mTime < 0.5f) { audioDialogManage.tooShort();
			 * isRecordering=false; mAudioManage.cancel();
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case MSG_VOICE_CHANGE:
				audioDialogManage.updateVoiceLevel(mAudioManage.getVoiceLevel(7));
				break;
