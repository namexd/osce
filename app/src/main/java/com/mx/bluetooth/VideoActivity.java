...
import com.mx.bluetooth.bean.VideoBean;
...
public class VideoActivity extends BaseActivity implements Callback {
	...
	private String mNvrIp;
	private int mNvrPort;
	...
	private void getNetVideoMessage() {
		...
		System.out.println("room_id=" + Utils.getSharedPrefrences(VideoActivity.this, "room_id"));
		String room_id = Utils.getSharedPrefrences(VideoActivity.this, "room_id");
		String exam_id = Utils.getSharedPrefrences(VideoActivity.this, "exam_id");
		String teacher_id = Utils.getSharedPrefrences(VideoActivity.this, "user_id");
		String url = BaseActivity.mSUrl + Constant.GET_VIDEO + "?room_id=" + room_id + "&exam_id=" + exam_id
			...
			public void onResponse(VideoBean infor) {
				try {
					if (infor.getCode() == 1) {
						...
						VideoActivity.this.mNvrIp = data.get(0).getIp();
						VideoActivity.this.mNvrPort = data.get(0).getRealport();
						VideoActivity.this.mNvrName = data.get(0).getUsername();
						VideoActivity.this.mNvrPassword = data.get(0).getPassword();
						VideoActivity.this.mChannel = data.get(0).getChannel();
						if (!initeActivity()) {
					...
					Toast.makeText(VideoActivity.this, "获得摄像头失败", Toast.LENGTH_SHORT).show();
				}

			}
		}, VideoActivity.this.errorListener());

		VideoActivity.this.executeRequest(gradeRequest);
	}
	 ...
	 * Button.OnClickListener() {
	 * 
	 * public void onClick(View v) { try { if(m_iLogID < 0) { Log.e(TAG,
	 * "please login on a device first"); return ; } if(m_iPlaybackID < 0) {
	 * if(m_iPlayID >= 0 ) { Log.i(TAG, "Please stop preview first"); return; }
	 * PlaybackCallBack fPlaybackCallBack = getPlayerbackPlayerCbf(); if
	 ...
	 * new String("ch0001_01000000154000100")); if(m_iPlaybackID >= 0) {
	 ...
	 * if(!HCNetSDK.getInstance().NET_DVR_PlayBackControl_V40(m_iPlaybackID,
	 * PlaybackControlCommand.NET_DVR_PLAYSTART, null, 0, struPlaybackInfo)) {
	 * Log.e(TAG, "net sdk playback start failed!"); return ; }
	 * m_oPlaybackBtn.setText("Stop"); int nProgress = -1; while(true) {
	 ...
	 * 
	 * 
	 * } else { Log.i(TAG, "NET_DVR_PlayBackByName failed, error code: " +
		...
		public void onClick(View v) {
			try {
				((InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(
						VideoActivity.this.getCurrentFocus().getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
				if (m_iLogID < 0) {
	...
	private void startSinglePreview() {
		...
		previewInfo.lChannel = Integer.parseInt(mChannel);
	...
	private int loginDevice() {
		...
		int iLogID = HCNetSDK.getInstance().NET_DVR_Login_V30(mNvrIp, mNvrPort, mNvrName, mNvrPassword,
