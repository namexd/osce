...
function course_module(){
    $('#submit-btn').click(function(){
        ...
        if(total != parseInt($('#total').val())){
            var scores = parseInt($('#total').val()) - parseInt(total),
        ...
        //折算率小数格式判断
        var rate_choose = $('#rate_choose option:selected').val();
        var rate_flag = false,
            sp_rate_flag = false,
            reg = new RegExp("^([0-9]+([.][0-9]+)|[0-9]+)$");
        if(rate_choose == 3||rate_choose == 2) {
            $('#judgement tbody').find('.rate').each(function(key,elem){
            ...
            if(false==rate_flag) {
                layer.alert('折算率未填写/折算率必须为小数或整数');
                return false;
            }
            if($('#checkbox_div').find('.check_icon').hasClass('check')){
                $('#special-score tbody').find('.special_rate').each(function(key,elem){
                    if(reg.test(parseFloat($(elem).val()))){
                        sp_rate_flag = true;
                        return true;
                    }else {
                        sp_rate_flag = false;
                ...
                if(false==sp_rate_flag) {
                    layer.alert('特殊评分折算率未填写/特殊评分折算率必须为小数或整数');
                    return false;
                }
            }
        }
    ...
    $('#add-new').click(function(){
        ...
        html += '</select>'+
                '<label class="rate_box">折算率：<input type="text" style="width:50px;border:1px solid" class="rate" name="score['+index+'][rate]"></label>'+
                '</td>'+
                '<td>'+
       ...
       /*todo:gaodapeng 2016.06.03*/

        var select=$('#rate_choose option:selected').val();
        switch (select){
            case '0':$('.rate_box').css('display','none');
                break;
            case '1':$('.rate_box').css('display','none');
                break;
            case '2':$('.rate_box').css('display','none');
                break;
            case '3':$('.rate_box').css('display','inline-block');
        }
    });
    ...
    $('#judgement tbody').on('click','.fa-plus',function(){
        ...
        html += '</select>'+
                '</td>'+
                /*'<td>' +
                '<select class="ability-tag"><option value="能力标签">能力标签</option></select>'+
                '</td>'+*/
                '<td>'+
                '<a href="javascript:void(0)"><span class="read state2 detail"><i class="fa fa-trash-o fa-2x"></i></span></a>'+
        ...
        change.after('<span class="each_score">'+parseInt(total)+'</span>');
...
*/
        ...
        if(thisElement.attr('child')==undefined){
            ...
            $('#judgement tbody tr').each(function(key,elem){
                if($(elem).attr('child')==undefined){
                    ...
                    $(elem).find('td').eq(2).find('input').attr('name','score['+parent+'][rate]');
                    $(elem).find('td').eq(1).find('input').eq(0).attr('name','content['+parent+'][id]');
                    $(elem).find('td').eq(1).find('input').eq(1).attr('name','content['+parent+'][title]');
                    parent += 1;
                }else{
                    ...
                    $(elem).find('td').eq(1).find('input').eq(0).attr('name','content['+parent_p+']['+child+'][id]');
                    $(elem).find('td').eq(1).find('input').eq(1).attr('name','content['+parent_p+']['+child+']['+child+']');
                    $(elem).find('td').eq(1).find('input').eq(2).attr('name','description['+parent_p+']['+child+']');
                    child += 1;
                }
            });
                    ...
                    if(data.code == 1){
                        ...
                        for(var i in res){
                           ...
                           if((res[i].sort).split('-')[1] == undefined){
                                index++;
                               var select = $('#rate_choose option:selected').val();
                               var flag = (select==3?'line-block':'none');
                               //添加父级dom
                               ...
                               html += '</select>'+
									   ...
									   '<label class="rate_box" style="display:'+flag+'">折算率：<input type="text" style="width:50px;border:1px solid" class="rate" name="score['+index+'][rate]"></label>'+
                                       '</td>'+
                                       '<td>'+
                                       '<a href="javascript:void(0)"><span class="read  state2 detail"><i class="fa fa-trash-o fa-2x"></i></span></a>'+
                                       '<a href="javascript:void(0)"><span class="read  state1 detail"><i class="fa fa-arrow-up parent-up fa-2x"></i></span></a>'+
    ...
    /**************************************************************zhoufuxiang <zhoufuxiang@163.com> 2016-05-07************************************************************************************/
    /**
     * 特殊评分checkbox
     * @author mao
     * @version 3.4
    ...
    $('#add-special-score').click(function() {
        ...
        html = '<tr>'+
            ...
            '<input type="hidden" class="special_rate" name="special_score['+index+'][rate]" />'+
            '</td>'+
           /* '<td>'+
             '<select name="ability-tag" id="ability-tag"><option value="能力标签">能力标签</option></select>'+
            '</td>'+*/
            '<td>'+
            '<a href="javascript:void(0)"><span class="read state2 detail"><i class="fa fa-trash-o fa-2x"></i></span></a>'+
            '</td>'+
...
function rate() {
    ...
    switch (select){
        case '0':
            break;
        case '1':
            $('#rate_score_box').css('display','none');
            $('#all_rate_box').css('display','none');
            $('.rate_box').css('display','none');
            $('#special_rate_box').css('display','none');
            //$('#choose').val(select);
            //rate_score();
            break;
        case '2':
            $('#rate_score_box').css('display','inline-block');
            $('#all_rate_box').css('display','inline-block');
            $('.rate_box').css('display','none');
            $('#special_rate_box').css('display','none');
            //$('#choose').val(select);
            break;
        case '3':
            $('#rate_score_box').css('display','inline-block');
            $('#all_rate_box').css('display','none');
            $('.rate_box').css('display','inline-block');
            $('#special_rate_box').css('display','inline-block');
            //$('#choose').val(select);
            break;
    }
    ...
    $("body").on('blur','#special_rate',function(){
        rate_score();
    });
    $("body").on('click','.fa-plus',function(){
        rate_score();
    });
    $("body").bind('click','.fa-trash-o',function(){
        rate_score();
    });
}
/***
...
function rate_score(){
    ...
    var total = 0;
    if(2==select){
        total = $('#all_rate').val()*$('#total').val();
        $('.rate').val($('#all_rate').val());
        $('.special_rate').val($('#all_rate').val());
    }else if(3==select){
        $('#judgement tbody').find('tr').each(function(key,elem) {
            if($(elem).attr('parent')!=undefined){
                var each_score = parseInt($(elem).find('td').eq(2).find('span').text());
                var each_rate  = $(elem).find('td').eq(2).find('input').val();
                total += (each_score*each_rate);
            }
        });
        $('.special_rate').val($('#special_rate').val());
    }else {
        total = $('#total').val();
        $('.rate').val("1");
        $('.special_rate').val("1");
    ...
    if (!isNaN(total)&&0!==total){
        $('#rate_score').val(parseFloat(total).toFixed(2));
    }else {
        $('#rate_score').val("");
    }
}