...
function exam_assignment_add(){
    ...
    $(".check_margin").click(function(){
        if ($(this).parent().find("input").val() != 1) {
            ...
            $('.grading-normal').show();
            $('.grading').find('select').html('<option value="1">统一设置各阶段考试顺序</option><option value="2">单独设置各阶段考试顺序</option>');
    ...
    $('#gradation_order').on('blur', function() {
        ...
        if(($('.check_margin').parent().find("input").attr('checked')=='checked') && $('.grading select').val() == 2 && $('#gradation_order').val() < 20) {
            for(var i = 1; i <= value; i++) {
                html += '<tr>'+
                            '<td>阶段'+i+'</td>'+
                            '<td>'+
...
function smart_assignment(){
    ...
    $('#send-info').click(function() {
        ...
        $.ajax({
            ...
            error:function(data) {
                layer.msg('通讯失败！',{skin:'msg-error',icon:1},function(its){
                    layer.close(its);
                });
            }
        });
...
function station_assignment(){
    ...
    function InitData(data) {
        ...
        for(var i in data) {
            ...
            html += '<div class="form-group">'+
                        ...
                        '<div class="col-sm-10">'+
                            '<div class="row">'+
                                '<div class="col-sm-1"><label class="control-label" order="'+data[i].order+'">第'+data[i].order+'站</label></div>'+
                                '<div class="col-sm-3"><label class="control-label">站名：</label></label><input class="form-control station-rename" name="name" value="'+data[i].name+'" style="display: inline-block;width: 180px;"></div>'+
                                '<div class="col-sm-4">'+
    ...
    $('.station-container').on('change', '.select-stage', function() {
        ...
        req['exam_id'] = examId;
        req['name'] = $(this).parent().siblings('.col-sm-3').find('input').val();
        req['order'] = $(this).parent().siblings('.col-sm-1').find('label').attr('order');
    ...
    //考站名称修改
    $('.station-container').on('blur', '.station-rename', function() {
        //考站dom
        var $that = $(this).parent().parent().parent().parent(),
            req = {};
        ...
        req['name'] = $(this).val();
        req['order'] = $(this).parent().siblings('.col-sm-1').find('label').attr('order');
        req['exam_gradation_id'] = $that.find('.select-stage').val();
        req['type'] =  $that.find('.select-stage').attr('type');
        req['flow_id'] = $that.find('table').attr('station-id');
            ...
            success: function(res) {
                if(res.code !=1) {
                    ...
                    html += '<div class="form-group">'+
                                ...
                                '<div class="col-sm-10">'+
                                    '<div class="row">'+
                                        '<div class="col-sm-1"><label class="control-label" order="'+index+'">第'+(parseInt(index)+1)+'站</label></div>'+
                                        '<div class="col-sm-3"><label class="control-label">站名：</label></label><input class="form-control station-rename" name="name" style="display: inline-block;width: 180px;"></div>'+
                                        '<div class="col-sm-4">'+
                                                '<label class="control-label col-sm-4">阶段：</label>'+
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if(res.code != 1) {
                            ...
                            //更新考站显示
                            $('.station-container').find('.col-sm-1').each(function(key,elem) {
    ...
    $('.station-container').on('change', '.chioce-btn', function() {
        ...
        $name = $(this).parent().siblings('.col-sm-3').find('input').val();

        $.ajax({
            type:'get',
            url: pars.chioce_btn,
            data:{exam_id:examId, optional:$that.val(),flow_id:$that.parent().parent().parent().find('table').attr('station-id'),type:$that.attr('type'),name:$name},
            success: function(res) {
                if(res.code != 1) {
                    return true;
                } else {
...
function examiner_manage() {
    ...
    function initTable(datas) {
        var html="",
            statusArr = [];     //记录状态老师
        var data = datas.data,  //获取考试安排数据
            exam = datas.exam;  //获取考试信息
        ...
        for(var i in data){
            ...
            //dom准备
            html += '<tr class="tr-id-'+i+'" exam_screening_id="'+data[i].exam_screening_id+'" value="'+data[i].subject_id+'" data-id="'+data[i].station_id+'" stage-id="'+data[i].exam_gradation_id+'">'+
                        '<td>'+data[i].subject_title+'</td>'+
                        '<td>'+data[i].station_name+'</td>'+
                        '<td>'+typeToName[data[i].station_type]+'</td>'+
                        '<td style="width:281px;">'+
                            '<div class="col-sm-10">'+
                            '<select class="form-control custom-teacher"  name=""  multiple="multiple" '+(exam.status?'disabled':'')+'>'+str_teacher+
                            '</select>'+
                            '</div>'+
                        '</td>'+
                        '<td style="width:281px;">'+
                            '<div class="col-sm-10">'+
                            '<select class="form-control custom-sp"  name="" '+control+'  multiple="multiple" '+(exam.status?'disabled':'')+'>'+str_sp+
                            '</select>'+
                            '</div>'+
                        '</td>'+
                        '<td>'+
    ...
    $('#save-data').click(function() {
        ...
        $('#add-basic tbody').find('tr').each(function(key) {
            ...
            //老师列表数组
            arr.push({exam_screening_id: $that.attr('exam_screening_id'), exam_gradation_id: $that.attr('stage-id'), subject_id:$that.attr('value'), station_id: $that.attr('data-id'), teacher:$that.find('.custom-teacher').val(), sp_teacher: $that.find('.custom-sp').val()});
