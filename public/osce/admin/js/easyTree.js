...
(function ($) {
    $.fn.EasyTree = function (options) {
        ...
        var createInput = $('<div class="input-group"><input type="text" style="color:#666" class="form-control"><span class="input-group-btn"><button type="button" class="btn btn-default btn-success confirm"></button> </span><span class="input-group-btn"><button type="button" class="btn btn-default cancel"></button> </span> </div> ');
        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function() {
                ...
                if($(this).is('li:has(ul)')) {
                    ...
                    $(children).remove();
                    text = $(this).text();
                    $(this).html('<span><span class="glyphicon"></span><a class="nou" href="javascript: void(0);"></a> </span>');
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        var item = $('<li><span><span class="glyphicon glyphicon-file"></span><a class="nou" href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        $(item).find(' > span > span').attr('title', options.i18n.collapseTip);
                        $(item).find(' > span > a').attr('title', options.i18n.selectTip);
                        if (selected.length <= 0) {
                        	var val=$(createInput).find('input').val();
                        	$.ajax('/msc/admin/resources-manager/add-resources-tools-cate',{
					            type: 'post',
					            data: {name:val},
            ...
            if (options.editable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .edit > button').attr('title', options.i18n.editTip).click(function () {
                    ...
                    if (selected.length <= 0) {
                        ...
                        var val=$(editor).val();
                        //console.log(id+","+val+"..."+val.length);
                        $(editor).keydown(function (e) {
                            if (e.which == 13) {
                                if ($(editor).val() !== '') {
			                        val=$(editor).val();
                                	//console.log(id+","+val+"..."+val.length);
                                	$.ajax('/msc/admin/resources-manager/edit-resources-tools-cate',{
							            type: 'post',
							            data: {id:id,name:val},
							            success:function(data) {
                ...
                $(easyTree).find('.easy-tree-toolbar .remove > button').attr('title', options.i18n.deleteTip).click(function () {
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {
                        	var  id = $(selected).attr("id");
                        	$.ajax('/msc/admin/resources-manager/del-resources-tools-cate',{
					            type: 'post',
					            data: {id:id},
					            success:function(data) {
