...
function theory_validate(){
                        ...
                        $.ajax({
                            ...
                            success:function(res){
                                if(res.code == 102){
                                    ...
                                    if(res.data.controlMark !=-1){
                                        var html  = '<div class="admissionBox font20 marb_25">'+
                                                     '<span class="admissionNum">由于该考生已:'+res.data.reason+'</span>' +
                                                     '</div>';
                                        $(".admissionNum").append(html);
                                        $(".goTest").html('下一个');

                                        $(".goTest").attr("controlMark",res.data.controlMark);
                                    }
                                    clearInterval(timer);
                                }
                            }
    ...
    $(".goTest").click(function(){
        ...
        var examId = $(".allData").attr("examId");
        if($(this).attr("controlMark")>0){
            $.ajax({
                url:"/osce/pad/dispose-aberrant-exam?student_id="+studentId+"&station_id="+stationId+"&user_id="+userId,
                dataType:"json",
                cache:false,
                type:"get",
                success:function(res){
