...
function statistics_student_score(){
    //默认加载select
    var _local = JSON.parse(localStorage.getItem('stuScore')),   //本地存储数据Object
        $selectId ;

    //判断是否获取本地存储数据
    if(_local != null) {
        $selectId = _local.selectId;
    } else {
                ...
                $(res.list).each(function(i){
                    if(this.type==3){
                            ...
                            $(".subjectBody").append('<tr>' +
                                '<td>'+( i+1 )+'</td>' +
                                '<td>'+this.paperName+'</td>' +
                                '<td>'+this.Pmins+'分钟</td>' +
                                '<td>'+this.timeAvg+'</td>' +
                                '<td>'+(this.scoreAvg?this.scoreAvg:'0.00')+'分</td>' +
                                '<td>'+this.time+'</td>' +
                                '<td>'+(this.score?this.score:'0.00')+'分</td>' +
                                '<td>' +
                                '<a href="/osce/admin/testscores/student-subject-list?examid='+examId+'&stuname='+stuname+'&subject='+this.paperName+'&student_id='+studentId+'&subid='+this.paper_id+'&sign=paper">' +
                                '<span class="read state1 detail"><i class="fa fa-cog fa-2x"></i></span>' +
                                '</a>' +
                                '<a href="/osce/admin/examanswer/student-answer?student_id='+this.student_id+'&flag='+this.flag+'">' +
                        ...
                        $(".subjectBody").append('<tr>' +
                            ...
                            '<td>'+this.mins+'分钟</td>' +
                            '<td>'+this.timeAvg+'</td>' +
                            '<td>'+this.scoreAvg+'分</td>' +
                            '<td>'+this.time+'</td>' +
                            '<td>'+(this.score?this.score:'0.00')+'分</td>' +
                            '<td>' +
                            '<a href="/osce/admin/testscores/student-subject-list?examid='+examId+'&stuname='+stuname+'&subject='+this.title+'&student_id='+studentId+'&subid='+this.id+'&sign=subject">' +
                            '<span class="read state1 detail"><i class="fa fa-cog fa-2x"></i></span>' +
                            '</a>' +
                            '<a href="/osce/admin/exam/exam-result-detail?exam_result_id='+this.result_id+'&flag='+this.flag+'">' +
                            '<span class="read state1 detail"><i class="fa fa-search fa-2x"></i></span>' +
                            '</a>' +
                            '</td></tr>')
                    }
